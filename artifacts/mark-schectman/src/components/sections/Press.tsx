...
export function Press() {
  ...
  return (
    <section id="press" className="py-24 bg-background relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 md:px-12">
        ...
        <div className="grid md:grid-cols-3 gap-px bg-border mb-16">
          {featuredQuotes.map((clipping, i) => (
            <motion.div
              ...
              className="bg-background p-10 relative group hover:bg-secondary transition-colors"
            >
              <div className="text-6xl font-serif text-primary/15 leading-none mb-4 select-none">&ldquo;</div>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-10">
          {regularClippings.map((clipping, i) => (
            <motion.div
              ...
              className="flex gap-6 items-start py-6 border-b border-border group"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {clipping.author && (
                  <p className="text-sm text-primary font-medium mt-3">&mdash; {clipping.author}</p>
                )}
              </div>
            </motion.div>
          ))}
        ...
        {cocktailClipping && (
          <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="mt-8 border border-primary/20 bg-primary/5 p-8 flex flex-col sm:flex-row gap-6 items-start sm:items-center"
          >
